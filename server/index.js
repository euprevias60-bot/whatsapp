...
async function getOrCreateSession(userId, io) {
  ...
  client.on('message_create', async (msg) => {
    if (msg.fromMe) {
      ...
      // Se o texto for exatamente o que o robô ia enviar, ignoramos a trava
  ...
  client.on('message', async msg => {
    ...
    try {
      ...
      // Salva o texto antes de enviar para o 'message_create' reconhecer
      session.lastBotResponses.set(chatId, response);
...
server.listen(PORT, () => {
  ...
  console.log(`-----------------------------------------`);
  console.log(`DEPLOYMENT VERSION: V3.1 (ROLE CLARITY)`);
...
server.listen(PORT, () => {
  console.log(`Server running on port ${PORT}`);
  console.log(`-----------------------------------------`);
  console.log(`DEPLOYMENT VERSION: V2.5 (FORCED REFRESH)`);
  console.log(`DEPLOY TIME: ${new Date().toLocaleString()}`);
  console.log(`-----------------------------------------`);
});
