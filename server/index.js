...
async function getOrCreateSession(userId, io) {
  ...
  const session = {
    ...
    chatHistories: new Map(), // Mapeia chatId -> array de histórico do Gemini
    lastBotResponses: new Map() // Mapeia chatId -> último texto enviado pelo robô
  };
  ...
  const HISTORY_TIMEOUT = 30 * 60 * 1000; // Resetar histórico após 30 min de silêncio
  ...
  client.on('message_create', async (msg) => {
    try {
      if (msg.fromMe) {
        // Special check for broadcast/status to avoid errors
        if (msg.to && (msg.to.includes('status') || msg.to.includes('broadcast'))) return;
        ...
        const body = msg.body;
        const expectedText = session.lastBotResponses.get(chatId);

        if (expectedText === body) {
          session.lastBotResponses.delete(chatId);
          return;
        }

        console.log(`Manual intervention detected in ${chatId}. Pausing bot for 5 mins.`);
        session.humanInteractions.set(chatId, Date.now());
        // Limpa histórico ao intervir manualmente para não confundir o robô depois
        session.chatHistories.delete(chatId);
      }
    } catch (err) {
  ...
  client.on('message', async msg => {
    try {
      ...
      if (msg.fromMe || msg.from === 'status@broadcast' || msg.from.includes('broadcast')) return;
      ...
      // Resetar histórico se passou muito tempo (opcional)
      // if (now - lastHumanTime > HISTORY_TIMEOUT) session.chatHistories.delete(chatId);

      if (now - lastHumanTime < INACTIVITY_TIMEOUT) return;

      await chat.sendStateTyping();

      // Busca histórico existente
      const history = session.chatHistories.get(chatId) || [];

      // Gera resposta com contexto
      const result = await session.aiAgent.generateResponse(msg.body, history);

      session.lastBotResponses.set(chatId, result.text);
      session.chatHistories.set(chatId, result.history);

      await chat.sendMessage(result.text);
      console.log(`Sent reply for user ${userId} to ${msg.from} (Context enabled)`);
    } catch (err) {
